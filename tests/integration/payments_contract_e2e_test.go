...
func TestSelectDefaultPaymentMethodUnauthorized(t *testing.T) {
	...
	request, writer := testharness.MakeRequest(http.MethodPost, "/api/v2/payments/stripe/methods/select-default", requestBody, "invalid-token")
...
func TestGetPaymentMethodsUnauthorized(t *testing.T) {
	request, writer := testharness.MakeRequest(http.MethodGet, "/api/v2/payments/stripe/methods", nil, "invalid-token")
	assert.Equal(t, http.StatusUnauthorized, writer.Code)
